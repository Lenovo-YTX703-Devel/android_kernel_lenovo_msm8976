...
/*fgy add start*/
struct bulkin_size AX88772B_BULKIN_SIZE[] = {
	/* 2k */
	{2048, 0x8000, 0x8001},
	/* 4k */
	{4096, 0x8100, 0x8147},
...
struct hwinit CICADA_FAMILY_HWINIT[40] = {
...
struct hwinit AGERE_FAMILY_HWINIT[] = {
...
MODULE_PARM_DESC(bsize, "Maximum transfer size per bulk");
 ...
 void ax88772b_link_reset(void *data);
 void ax88772a_link_reset(void *data);
 void ax88772_link_reset(void *data);
#else
 void ax88772b_link_reset(struct work_struct *work);
 void ax88772a_link_reset(struct work_struct *work);
 void ax88772_link_reset(struct work_struct *work);
#endif
 int ax88772a_phy_powerup(struct usbnet *dev);
 void ax8817x_mdio_write_le(struct net_device *netdev, int phy_id,
				  int loc, int val);
 int ax8817x_mdio_read_le(struct net_device *netdev, int phy_id, int loc);
 int ax88772b_set_csums(struct usbnet *dev);

/* ASIX AX8817X based USB 2.0 Ethernet Devices */

 int ax8817x_read_cmd(struct usbnet *dev, u8 cmd, u16 value, u16 index,
			    u16 size, void *data)
{
 ...
 int ax8817x_write_cmd(struct usbnet *dev, u8 cmd, u16 value, u16 index,
			     u16 size, void *data)
{
	return usb_control_msg(
		dev->udev,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 20)
 void ax8817x_async_cmd_callback(struct urb *urb, struct pt_regs *regs)
#else
 void ax8817x_async_cmd_callback(struct urb *urb)
#endif
{
 ...
 int ax8817x_set_mac_addr(struct net_device *net, void *p)
{
 ...
 void ax88178_status(struct usbnet *dev, struct urb *urb)
{
 ...
 void ax8817x_status(struct usbnet *dev, struct urb *urb)
{
	struct ax88172_int_data *event;
	int link;
 ...
 void ax88772_status(struct usbnet *dev, struct urb *urb)
{
	struct ax88172_int_data *event;
 ...
 void ax88772a_status(struct usbnet *dev, struct urb *urb)
{
	struct ax88172_int_data *event;
 ...
 int ax88772b_stop(struct usbnet *dev)
{
	u16 *medium;
 ...
 int ax88772b_reset(struct usbnet *dev)
{
	int ret, i;
	void *buf;
...
void ax88772b_status(struct usbnet *dev, struct urb *urb)
{
 ...
 void ax88772c_status(struct usbnet *dev, struct urb *urb)
{
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
	struct ax88172_int_data *event;
	int link;
				    ...
				    u16 size, void *data)
{
 ...
 void ax8817x_set_multicast(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
 ...
 void ax88178_set_multicast(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
 ...
 void ax88772b_set_multicast(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
 ...
 int ax8817x_mdio_read(struct net_device *netdev, int phy_id, int loc)
{
...
ax8817x_mdio_write_le(struct net_device *netdev, int phy_id, int loc, int val)
{
 ...
 void ax8817x_swmii_mdio_write_le(struct net_device *netdev,
...
ax88772b_mdio_write_le(struct net_device *netdev, int phy_id, int loc, int val)
{
 ...
 int ax88772_suspend(struct usb_interface *intf,
...
#endif
{
 ...
 int ax88772b_suspend(struct usb_interface *intf,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 10)
			    pm_message_t message)
#else
			    u32 message)
...
//	u32 tmp32;
 ...
 int ax88772_resume(struct usb_interface *intf)
{
	struct usbnet *dev = usb_get_intfdata(intf);
 ...
 int ax88772b_resume(struct usb_interface *intf)
{
	struct usbnet *dev = usb_get_intfdata(intf);
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
...
int ax88172_link_reset(struct usbnet *dev)
{
	u16 lpa;
	u16 adv;
	u16 res;
...
ax8817x_set_wol(struct net_device *net, struct ethtool_wolinfo *wolinfo)
{
 ...
 int ax8817x_get_eeprom_len(struct net_device *net)
{
			      ...
			      struct ethtool_eeprom *eeprom, u8 *data)
{
 ...
 void ax8817x_get_drvinfo(struct net_device *net,
				struct ethtool_drvinfo *info)
{
 ...
 int ax8817x_get_settings(struct net_device *net, struct ethtool_cmd *cmd)
{
	struct usbnet *dev = netdev_priv(net);
	return mii_ethtool_gset(&dev->mii, cmd);
}

 int ax8817x_set_settings(struct net_device *net, struct ethtool_cmd *cmd)
{
	struct usbnet *dev = netdev_priv(net);
...
/* We need to override some ethtool_ops so we require our
 ...
 struct ethtool_ops ax8817x_ethtool_ops = {
 ...
 int ax8817x_ioctl(struct net_device *net, struct ifreq *rq, int cmd)
{
	struct usbnet *dev = netdev_priv(net);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 29)
 const struct net_device_ops ax88178_netdev_ops = {
	.ndo_open			= axusbnet_open,
	.ndo_stop			= axusbnet_stop,
	.ndo_start_xmit	= axusbnet_start_xmit,
	.ndo_tx_timeout	= axusbnet_tx_timeout,
 ...
 int ax8817x_bind(struct usbnet *dev, struct usb_interface *intf)
{
	int ret = 0;
	void *buf;
	int i;
...
out1:
 ...
 struct ethtool_ops ax88772_ethtool_ops = {
	.get_drvinfo		= ax8817x_get_drvinfo,
	.get_link		= ethtool_op_get_link,
	.get_msglevel		= axusbnet_get_msglevel,
	.set_msglevel		= axusbnet_set_msglevel,
 ...
 int ax88772_bind(struct usbnet *dev, struct usb_interface *intf)
{
	int ret;
	void *buf;
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
 ...
 int ax88772a_phy_powerup(struct usbnet *dev)
{
 ...
 int ax88772a_bind(struct usbnet *dev, struct usb_interface *intf)
{
	int ret = -EIO;
	void *buf;
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
 ...
 int ax88772b_set_csums(struct usbnet *dev)
{
 ...
 u32 ax88772b_get_tx_csum(struct net_device *netdev)
{
	struct usbnet *dev = netdev_priv(netdev);
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
 ...
 u32 ax88772b_get_rx_csum(struct net_device *netdev)
{
	struct usbnet *dev = netdev_priv(netdev);
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
 ...
 int ax88772b_set_rx_csum(struct net_device *netdev, u32 val)
{
	struct usbnet *dev = netdev_priv(netdev);
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
 ...
 int ax88772b_set_tx_csum(struct net_device *netdev, u32 val)
{
	struct usbnet *dev = netdev_priv(netdev);
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
...
#endif
 struct ethtool_ops ax88772b_ethtool_ops = {
	.get_drvinfo		= ax8817x_get_drvinfo,
	.get_link		= ethtool_op_get_link,
	.get_msglevel		= axusbnet_get_msglevel,
	.set_msglevel		= axusbnet_set_msglevel,
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 29)
 const struct net_device_ops ax88772b_netdev_ops = {
	.ndo_open		= axusbnet_open,
	.ndo_stop		= axusbnet_stop,
	.ndo_start_xmit		= axusbnet_start_xmit,
	.ndo_tx_timeout		= axusbnet_tx_timeout,
 ...
 int ax88772b_bind(struct usbnet *dev, struct usb_interface *intf)
{
	int ret;
	void *buf;
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
...
err_out:
 ...
 void ax88772b_unbind(struct usbnet *dev, struct usb_interface *intf)
{
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
...
ax88178_media_check(struct usbnet *dev, struct ax88178_data *ax178dataptr)
{
 ...
 void Vitess_8601_Init(struct usbnet *dev, int state)
{
	u16 reg;
...
SKIPPHYSETTING:
 ...
 int ax88178_bind(struct usbnet *dev, struct usb_interface *intf)
{
	int ret;
	void *buf;
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
...
error_out:
 ...
 void ax88178_unbind(struct usbnet *dev, struct usb_interface *intf)
{
	struct ax88178_data *ax178dataptr = (struct ax88178_data *)dev->priv;
 ...
 int ax88772_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	u8  *head;
	u32  header;
	char *packet;
 ...
 struct sk_buff *ax88772_tx_fixup(struct usbnet *dev, struct sk_buff *skb,
					gfp_t flags)
{
...
ax88772b_rx_checksum(struct sk_buff *skb, struct ax88772b_rx_header *rx_hdr)
{
 ...
 int ax88772b_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
 ...
 void ax88772b_link_reset(struct work_struct *work)
{
 ...
 int ax88178_set_media(struct usbnet *dev)
{
	int	ret;
	struct ax88178_data *ax178dataptr = (struct ax88178_data *)dev->priv;
	int media;
 ...
 int ax88178_link_reset(struct usbnet *dev)
{
	return ax88178_set_media(dev);
}

 int ax_suspend(struct usb_interface *intf,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 10)
			pm_message_t message)
#else
			u32 message)
#endif
{
 ...
 int ax_resume(struct usb_interface *intf)
{
	struct usbnet *dev = usb_get_intfdata(intf);
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
 ...
 const struct driver_info belkin178_info = {
 ...
 const struct driver_info dlink_dub_e100b_772b_info = {
	.description = "D-Link DUB-E100 USB 2.0 Fast Ethernet Adapter",
	.bind = ax88772b_bind,
 ...
 const struct driver_info ax88772a_info = {
 ...
 const struct driver_info ax88772b_info = {
 ...
 const struct driver_info ax88772c_info = {
...
MODULE_DEVICE_TABLE(usb, products);
 ...
 int __init asix_init(void)
{
	printk("fgy asix_init \n");
	return usb_register(&asix_driver);
}
module_init(asix_init);

 void __exit asix_exit(void)
{
