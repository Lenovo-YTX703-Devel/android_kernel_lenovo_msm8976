...
#define DRIVER_VERSION		"22-Aug-2005"
 ...
 void axusbnet_unlink_rx_urbs(struct usbnet *);
...
//fgy add
extern struct bulkin_size AX88772B_BULKIN_SIZE[];
extern struct hwinit CICADA_FAMILY_HWINIT[];
extern struct hwinit CICADA_V2_HWINIT[];
extern struct hwinit CICADA_V2_ASIX_HWINIT[];
extern struct hwinit AGERE_FAMILY_HWINIT[];
...
int axusbnet_get_endpoints(struct usbnet *dev, struct usb_interface *intf)
{
 ...
 void intr_complete(struct urb *urb, struct pt_regs *regs);
#else
 void intr_complete(struct urb *urb);
#endif

 int init_status(struct usbnet *dev, struct usb_interface *intf)
{
	char		*buf = NULL;
	unsigned	pipe = 0;
	unsigned	maxp;
...
int axusbnet_change_mtu(struct net_device *net, int new_mtu)
{
 ...
 struct net_device_stats *axusbnet_get_stats(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	return &dev->stats;
}
...
/*-------------------------------------------------------------------------*/
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19)
 void rx_complete(struct urb *urb, struct pt_regs *regs);
#else
 void rx_complete(struct urb *urb);
#endif

 void rx_submit(struct usbnet *dev, struct urb *urb, gfp_t flags)
{
	struct sk_buff		*skb;
...
/*-------------------------------------------------------------------------*/
 ...
 inline void rx_process(struct usbnet *dev, struct sk_buff *skb)
{
...
/* unlink pending rx/tx; completion handlers do all other cleanup */
 ...
 int unlink_urbs(struct usbnet *dev, struct sk_buff_head *q)
{
	unsigned long		flags;
...
/* drivers may override default ethtool_ops in their bind() routine */
 struct ethtool_ops axusbnet_ethtool_ops = {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19)
 void tx_complete(struct urb *urb, struct pt_regs *regs)
#else
 void tx_complete(struct urb *urb)
#endif
{
	struct sk_buff		*skb = (struct sk_buff *) urb->context;
	struct skb_data		*entry = (struct skb_data *) skb->cb;
...
/* tasklet (work deferred from completions, in_irq) or timer */
 ...
 void axusbnet_bh(unsigned long param)
{
...
axusbnet_probe(struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	int				status;
	const char			*name;
	printk("fgy axusbnet_probe start\n");
 ...
 */
 ...
 int axusbnet_suspend(struct usb_interface *intf,
